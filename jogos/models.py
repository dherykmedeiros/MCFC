...
class Presenca(models.Model):
    ...
    usuario = models.ForeignKey(
        NewUser, on_delete=models.CASCADE, related_name="presencas"
    )
...
class DesempenhoJogador(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    jogo_dia = models.ForeignKey(
        Jogo, on_delete=models.CASCADE, related_name="desempenhos"
    )
    jogador = models.ForeignKey(
        Jogador, on_delete=models.CASCADE, related_name="desempenhos"
    )
